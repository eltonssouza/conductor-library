...
> Uma biblioteca organizada como uma trilha de Engenharia de Computação. Corpus **somente em inglês**, sem imagens. As listas de leitura do currículo ficam na pasta `00_`; os livros completos são agrupados por tópico nas pastas `01_`–`14_`. Veja `FILE_CONVENTIONS.md` para o padrão de nomenclatura e formatação, incluindo a política de **versionamento** ([`VERSION`](VERSION)).


## Documentos de referência

- [LIBRARY_INDEX.json](LIBRARY_INDEX.json) — catálogo legível por máquina (caminho, tier, stack, versão) que o Conductor consome
- [CONDUCTOR.md](CONDUCTOR.md) — como o app Conductor consome esta biblioteca (instalação/ingestão)
- [FILE_CONVENTIONS.md](FILE_CONVENTIONS.md) — padrão de nomenclatura de pastas/arquivos e formatação interna
- [ROLES_AND_ACRONYMS.md](ROLES_AND_ACRONYMS.md) — glossário de papéis técnicos e siglas
- [AGENTS_AND_SKILLS_BY_ROLE.md](AGENTS_AND_SKILLS_BY_ROLE.md) — prompt de Agent + Skill por papel

## Usado pelo Conductor

Este corpus é a memória **Library (RAG)** por trás do [Conductor](https://github.com/eltonssouza/conductor-main). Você não o instala à mão — o `cdt up` busca este repositório (`CONDUCTOR_LIBRARY_REPO@CONDUCTOR_LIBRARY_REF`, padrão `eltonssouza/conductor-library@main`), gera embeddings de cada `.md` com bge-m3 e o indexa no ChromaDB. Cada pasta de topo vira a `category` do RAG; o tier do frontmatter `software_dev` permite filtrar o corpus para material relevante ao desenvolvimento. Veja [CONDUCTOR.md](CONDUCTOR.md) para a integração completa e as variáveis de ambiente.

<!-- AUTO-INDEX:START -->

> **Gerado por `scripts/build_index.py` — não edite à mão.** Espelha o [LIBRARY_INDEX.json](LIBRARY_INDEX.json), o catálogo legível por máquina que o Conductor consome.

> **Versão da biblioteca:** `0.1.0` — de [`VERSION`](VERSION). Incremente o dígito _patch_ a cada mudança (`99` → próximo _minor_).
...
**Total: 136 arquivos de livro/lista de leitura**
