...
Chapter 4: Styles of the C&C Viewtype
Appendix to Section
A complete description of the simple PipeFilter system appears below. The Pipe and Role protocols adapted from the example in []). The protocols in this description are defined by a CSP process expression that indicates sequencing of events (�), internal choice (|~|), and external events ([]). "tick" represents a successfully terminated process. Events preceded by an underscore represent "initiated" events; others represent "observed" events.
Style:
Family PipeFilter = { Port Type OutputPort; Port Type InputPort;
Role Type Source; Role Type Sink;
Component Type Filter;
Connector Type Pipe = { Role src : Source = new Source extended with { Properties { protocolInterface : String = "Source = (_read � data?x � Source) |~| (_close � tick)" } } Role snk : Sink = new Sink extended with { Properties { protocolInterface : String = "Sink = (_write � Sink) |~| (_close � tick)" } } Properties { protocolGlue : String = "Buf(<>) = Source.write?x � Buf(<x>) [] Source.close � Closed(<>) Buf(S<>) = Source.write?y � Buf(<y>S<x>) [] Source.close � Closed(S<x>) [] Sink.read � _Sink.data!x � Buf(S) [] Sink.close � Killed Closed(S<x>) = Sink.read � _Sink.data!x � Closed(S) [] Sink.close � tick Closed(<>) = Sink.close � tick Killed = Source.write � Killed [] Source.close � tick" }
}; };
