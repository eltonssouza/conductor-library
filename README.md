...
> A library organized as a Computer Engineering track. **English-only** corpus, no images. Curriculum reading lists live in folder `00_`; full books are grouped by topic in folders `01_`–`14_`. See `FILE_CONVENTIONS.md` for the naming and formatting standard, including the **versioning** policy ([`VERSION`](VERSION)).
...
> **Library version:** `0.1.0` — from [`VERSION`](VERSION). Bump the patch digit on every change (`99` → next minor).
